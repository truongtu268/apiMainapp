...
Model.prototype = {
  ...
  create: function (attributes) {
    if(_.isArray(attributes)){
      var mapAttribute = _.map(attributes,function (attribute) {
        attribute.code = randToken.generate(sails.config.security.urlToken.length);
        return attribute;
      });
      return promisify(this.model.create)(mapAttribute);
    }
    attributes.code = randToken.generate(sails.config.security.urlToken.length);
  ...
  findOnePopulate: function (criteria,association, subcriteria) {
    return promisify(this.model.findOne(criteria).populate)(association, subcriteria);
  },
  ...
  recover: function (criteria, attributes) {
    attributes.isDelete = true;
    return promisify(this.model.update)(criteria, attributes);
  },
  delete: function (criteria,attributes) {
    attributes.isDelete = true;
    return promisify(this.model.update)(criteria, attributes);
  },
